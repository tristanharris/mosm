...
def m30
  ...
  if $R=29 && $F[48]=0 then $Jstr=$Jstr+" GRARGS FEASTING,";end
  if $R=29 && $F[48]=1 then $Jstr=$Jstr+" A SLEEPING GRARG,";end
  if $R=12 || $R=22 then $Jstr=$Jstr+" A PONY,";end
  if $R=64 then $Jstr=$Jstr+" A HERMIT,";end
  if $R=18 && $Estr[18]="N" then $Jstr=$Jstr+" AN OAK DOOR,";end
  if $R=59 && $F[68]=1 then $Jstr=$Jstr+" OGBAN (DEAD),";end
  ...
  $Vstr="";$Tstr="";$VB=0;$B=0
  for $I in 1..LEN($Istr)
  if MIDstr($Istr,$I,1)=" " && $Vstr="" then $Vstr=LEFTstr($Istr,$I-1);end
  if MIDstr($Istr,$I+1,1)<>" " && $Vstr<>"" then $Tstr=MIDstr($Istr,$I+1,LEN($Istr)-1);$I=LEN($Istr);end
...
begin
  ...
  if $VB>$NV && $B=0 then $Rstr="YOU CANNOT "+$Istr;end
  if !($B>$G || $B=0) then
  if !($VB=8 || $VB=9 || $VB=14 || $VB=17 || $VB=44 || $VB>54) then
  if $VB<$NV && $C[$B]<>0 then $Rstr="YOU DO NOT HAVE THE "+$Tstr;m30;end
end
end
  if $R=56 && $F[35]=0 && $VB<>37 && $VB<>53 then $Rstr=$X1str+" HAS GOT YOU!";m30;end
  if !($VB=44 || $VB=47 || $VB=19 || $VB=57 || $VB=49) then
  if $R=48 && $F[63]=0 then $Rstr=$X9str;m30;end
end
  ...
  if $R=56 && $F[35]=0 && $C[10]<>0 then $Rstr=$X1str+" GETS YOU!";$F[56]=1;end
...
def m800
  ...
  if $D=5 then $D=1;end
  if $D=6 then $D=3;end
  if !(NOT (($R=75 && $D=2) || ($R=76 && $D=4)) || $F[64]=1) then
  ...
  if $R=33 && $D=1 && $F[32]=0 then $Rstr="OGBAN'S BOAR BLOCK YOUR PATH";return;end
  if (($R=3 && $D=2) || ($R=4 && $D=4)) && $F[45]=0 then $Rstr=$X5str;return;end
  if $R=35 && $C[13]<>$R then $Rstr="THE ICE IS BREAKING!";return;end
  if $R=5 && ($D=2 || $D=4) then m4310;end
  ...
  if $R=48 && $D=4 && $F[70]=0 then $Rstr="THE DOOR DOES NOT OPEN";return;end
  if $R=40 && $F[47]=1 then $F[68]=1;end
  if $R=37 && $D=4 && $Estr[37]="EW" then $R=67;$Rstr="THE PASSAGE WAS STEEP!";return;end
  ...
  if $Kstr="W" && $D=4 then $R=R-1;end
	end;$Rstr="OK"
  if $R=$OM then $Rstr="YOU CANNOT GO THAT WAY";end
  if (($OM=75 && $D=2) || ($OM=76 && $D=4)) then $Rstr="OK. YOU CROSSED";end
  if $F[29]=1 then $F[39]=$F[39]+1;end
  if $F[39]>5 && $F[29]=1 then $Rstr="CPPUT IBWF XPSO PVU";m4260; $F[29]=0;$C[3]=81;end
  return
...
def m1290
  ...
  if $H=4177 || $H=5177 then $B=16;m2380;return;end
  if $B=38 then $Rstr="TOO HEAVY!";return;end
  if $B=4 && $F[43]=0 then $Rstr="IT IS FIRMLY NAILED ON!";return;end
  ...
  if $C[$B]=$R && $F[$B]=0 then $C[$B]=0;$Rstr="YOU HAVE THE "+$Tstr;end
  if $B=28 then $C[5]=81;end
  if $B=5 then $C[28]=0;end
  if $C[4]=0 && $C[12]=0 && $C[15]=0 then $F[54]=1;end
  if $B=8 && $F[30]=1 then $C[2]=0;end
...
def m1470
  ...
  if ($H=1443 || $H=1485) && $F[33]=0 then $Rstr="$B HMJNNFSJOH GSPN UIF EFQUIT";m4260;end
  if ($H=1443 || $H=1485) && $F[33]=1 then $Rstr="SOMETHING HERE...";$F[12]=0;end
  if $H=2479 || $H=2444 then $Rstr="THERE IS A HANDLE";end
  if $B=9 then $Rstr="UIF MBCFM SFBET 'QPJTPO'";m4260;end
  if $H=4055 then m3290;end
  if $H=2969 && $F[49]=1 then $Rstr="VERY UGLY!";end
  if $H=7158 || $H=7186 then $Rstr="THERE ARE LOOSE BRICKS";end
  if $R=49 then $Rstr="VERY INTERESTING!";end
  if $B=52 || $B=82 || $B=81 then $Rstr="INTERESTING!";end
  ...
  if $H=6865 || $H=6853 then $Rstr="THERE ARE NINE STONES";end
...
def m1750
  ...
  if $R=75 || $R=76 then $Rstr="HE DOES NOT WANT IT";end
  if $B=62 && $F[44]=0 then $Rstr="YOU HAVE RUN OUT!";end
  if ($H=7562 || $H=7662) && $F[44]>0 && $C[1]=0 then $Rstr="HE TAKES IT";$F[64]=1;end
  if $F[64]=1 then $F[44]=$F[44]-1;end
  if $B=1 then $Rstr="HE TAKES THEM ALL!";$C[1]=81;$F[64]=1;$F[44]=0;end
  if $H=2228 && $C[5]=81 then $Rstr=$XBstr+"NORTH";$C[28]=81;$R=12;end
  if ($H=2228 && $C[5]=81) || $H=225 then $Rstr=$XBstr+"NORTH";$R=12;end
  if ($H=1228 && $C[5]=81) || $H=125 then $Rstr=$XBstr+"SOUTH";$R=12;end
  if $R=7 || $R=33 then $Rstr="HE EATS IT!";$C[$B]=81;end
  if $H=711 then $F[46]=1;$Rstr="HE IS DISTRACTED";end
  if $H=385 || $H=3824 then $Rstr="THEY SCURRY AWAY";$C[$B]=81;$F[65]=1;end
  return
  $Rstr="YOU SAID IT"
  if $B=84 then $Rstr="YOU MUST SAY THEM ONE BY ONE!";return;end
  if $R<>47 || $B<71 || $B>75 || $C[27]<>0 then return;end
  if $B=71 && $F[60]=0 then $Rstr=$X7str;$F[60]=1;return;end
  if $B=72 && $F[60]=1 && $F[61]=0 then $Rstr=$X8str;$F[61]=1;return;end
  if $B=($F[52]+73) && $F[60]=1 && $F[61]=1 then $F[62]=1;return;end
  $Rstr="THE WRONG SACRED WORD!";$F[56]=1;return
end

def m1960
  if $B=5 || $B=10 then m1290;end
  return
  if $B=3 then $F[29]=1;$Rstr="ZPV BSF JOWJTJCMF";$F[55]=0;m4260;end
  if $B=20 then $F[51]=1;$Rstr="ZPV BSF EJTHVJTFE";$F[55]=0;m4260;end
  return
  if $B=2 || $B=14 then $Rstr="NOTHING TO TIE IT TO!";end
  if $H=7214 then $Rstr="IT IS TIED";$C[14]=72;$F[53]=1;end
  if $H=722 then $Rstr="OK";$F[40]=1;$C[2]=72;end
  return
  if $H=1547 && $F[38]=1 then $Rstr="ALL RIGHT";$R=16;end
  if $B=14 || $B=2 then $Rstr="NOT ATTACHED TO ANYTHING!";end
  if $H=5414 && $C[14]=54 then $Rstr="YOU ARE AT THE TOP";end
  if $H=7214 && $F[53]=1 then $Rstr="GOING DOWN";$R=71;end
  if $H=722 && $F[40]=1 then $R=71;$Rstr="IT IS TORN";$C[2]=81;$F[40]=0;end
  if $H=7114 && $F[53]=1 then $C[14]=71;$F[53]=0;$Rstr="IT FALLS DOWN-BUMP!";end
  return
  if $H=522 then $Rstr="OK";$F[30]=1;end
  if $B=1 || $B=62 || $B=5 || $B=28 || $B=11 || $B=24 then m1750;end
  if $H=416 then $Rstr="ZPV IBWF LFQU BGMPBU";$F[31]=1;m4260;return;end
  if $H=4116 then $Rstr="IT IS NOT BIG ENOUGH!";return;end
  if $B=18 || $B=7 then m2470;end
  if $B=13 then m2730;end
  if $B=19 then m3070;end
  if $B=10 then m2870;end
  if $B=16 || $B=6 then m2380;end
  return
  if $B=76 || $B=38 then m1470;end
  if $H=2030 then $F[9]=0;$Rstr="OK";end
  if $H=6030 then $Rstr="OK";$F[3]=0;end
  if $H=2444 || $H=1870 then $Rstr="YOU ARE NOT STRONG ENOUGH";end
  ...
  if ($B=23 || $H=6970) && $C[26]<>0 then $Rstr="OP NBUDIFT";m4260;end
  if $B=23 && $C[26]=0 then $Rstr="A BRIGHT "+$Vstr;$F[50]=1;end
  if $H=6970 && $C[26]=0 then $F[43]=1;$Rstr="IT HAS TURNED TO ASHES";end
  return
end

def m2380
  if ($B=16 || $B=6) && ($R=41 || $R=51) then $Rstr="YOU CAPSIZED!";$F[56]=1;end
  if $H=6516 && $C[16]=0 then $Rstr="IT IS NOW FULL";$F[34]=1;end
  if $H=656 then $Rstr="IT LEAKS OUT!";end
  return
  if $B<>22 || $R<>15 then $Rstr="DOES NOT GROW!";return;end
  $Rstr="OK";$F[34]=1
  return
end

def m2450
  if $B=22 && $F[37]=1 && $F[34]=1 then $Rstr=$X2str;$F[38]=1;m4260;end
  return
end

def m2470
  if $B=7 || $B=18 then $Rstr="THWACK!";end
...
def m2550
  ...
  return
  if ($B=3 && $F[29]=1) then $Rstr="TAKEN OFF";$F[29]=0;end
  if ($B=20 && $F[51]=1) then $Rstr="OK";$F[51]=0;end
  if $B=36 || $B=50 then m2950;end
  return
  if $H=3859 || $H=3339 || $H=1241 || $H=2241 || $H=751 then $Rstr="WITH WHAT?";end
  return
  if $H=2340 then $Rstr="IT GOES ROUND";end
  if $H=2445 then $Rstr="UIF HBUFT PQFQ, UIF QPPM FNQUJFT";$F[33]=1;m4260;end
  return
  if $R=14 || $R=51 then $Rstr="YOU HAVE DROWNED";$F[56]=1;end
  return
  $Rstr="HOW?";return
end

def m2730
  if $B=0 || $B>$G then return;end
  $C[$B]=$R;$Rstr="DONE"
  if $H=418 || $H=518 then $Rstr="YOU DROWNED!";$F[56]=1;end
  if $B=8 && $F[30]=1 then $C[2]=$R;end
  if $B=16 && $F[34]=1 then $Rstr="YOU LOST THE WATER!";$F[34]=0;end
  if $B=2 && $F[30]=1 then $F[30]=0;end
  return
  if $B=62 && $F[44]=0 then $Rstr="YOU DO NOT HAVE ANY";end
  if $H=5762 && $C[1]=0 && $F[44]>0 then m3230;end
  return
  if $B=0 || $B>$G then return;end
...
def m2870
  ...
  return
  if $B=0 || $B>$G then return;end
  if $B=5 || $B=24 then $Rstr="YUM YUM!";$C[$B]=81;end
  return
end

def m2950
  if $R=4 && $B=50 then $F[45]=1;$Rstr="YOU REVEALED A STEEP PASSAGE";end
  if $R=3 && $B=50 then $Rstr="YOU CANNOT MOVE RUBBLE FROM HERE";end
  if $H=7136 then $Rstr="THEY ARE WEDGED IN!";end
  return
  if ($B=67 || $B=68) && $C[9]=0 && $R=49 then $Rstr="OK";$F[47]=1;end
  return
  if $R<>27 || $B<>63 then return;end
...
def m3070
  if ($H=4864 || $H=4819) && $C[19]=0 then $Rstr=$X6str;$F[63]=1;m4260;end
  if $B=27 then m1290;end
  return
  if $H=7549 || $H=7649 then $Rstr="WHAT WITH?";end
  if $B=1 || $B=62 then m1750;end
  return
  if $H=4870 && $C[21]=0 then $Rstr="THE KEY TURNS!";$F[70]=1;end
  return
...
def m4830
  $LS=1;$LP=1
  for $I in 1..LEN($Jstr)
  if MIDstr($Jstr,$I,1)=" " && $LL>$EL then puts MIDstr($Jstr,$LP,$LS-$LP);$LL=$I-$LS;$LP=$LS+1;end
