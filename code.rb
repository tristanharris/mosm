...
120     IF F[I]=0 AND C[I]=R THEN Jstr=Jstr+" "+Pstr+" "+Ostr+","
130     NEXT I
140     IF R=29 AND F[48]=0 THEN Jstr=Jstr+" GRARGS FEASTING,"
150     IF R=29 AND F[48]=1 THEN Jstr=Jstr+" A SLEEPING GRARG,"
160     IF R=12 OR R=22 THEN Jstr=Jstr+" A PONY,"
170     IF R=64 THEN Jstr=Jstr+" A HERMIT,"
180     IF R=18 AND Estr[18]="N" THEN Jstr=Jstr+" AN OAK DOOR,"
190     IF R=59 AND F[68]=1 THEN Jstr=Jstr+" OGBAN (DEAD),"
200     IF Jstr<>"" THEN Jstr=", YOU CAN SEE"+Jstr
...
310     IF MIDstr(Istr,I,1)=" " AND Vstr="" THEN Vstr=LEFTstr(Istr,I-1)
320     IF MIDstr(Istr,I+1,1)<>" " AND Vstr<>"" THEN Tstr=MIDstr(Istr,I+1,LEN(Istr)-1):I=LEN(Istr)
330     NEXT I:IF Tstr="" THEN Vstr=Istr
340     IF LEN(Vstr)<3 THEN Vstr=Vstr+"O":GOTO 340
350     IF Vstr="PLAY" THEN Vstr="BLO"
360     Ustr=LEFTstr(Vstr,3)
370     FOR I=1 TO NV:IF MIDstr(Bstr,I*3-2,3)=Ustr THEN VB=I:I=NV
380     NEXT I:F[36]=0
390     m3330
400     FOR I=1 TO NO:READ Ostr:IF I<=G THEN m3350
410     IF Tstr=Ostr THEN B=I:I=NO
420     NEXT I
430     IF B=0 AND F[36]=0 AND Tstr>"" THEN Tstr=Tstr+"S":F[36]=1:GOTO 390
440     IF VB=0 THEN VB=NV+1
450     IF Tstr="" THEN Rstr="I NEED TWO WORDS"
460     IF VB>NV THEN Rstr="TRY SOMETHING ELSE"
470     IF VB>NV AND B=0 THEN Rstr="YOU CANNOT "+Istr
480     IF B>G OR B=0 THEN GOTO 510
490     IF VB=8 OR VB=9 OR VB=14 OR VB=17 OR VB=44 OR VB>54 THEN GOTO 510
500     IF VB<NV AND C[B]<>0 THEN Rstr="YOU DO NOT HAVE THE "+Tstr:GOTO 30
510     IF R=56 AND F[35]=0 AND VB<>37 AND VB<>53 THEN Rstr=X1str+" HAS GOT YOU!":GOTO 30
520     IF VB=44 OR VB=47 OR VB=19 OR VB=57 OR VB=49 THEN GOTO 540
530     IF R=48 AND F[63]=0 THEN Rstr=X9str:GOTO 30
...
660     IF R=41 THEN F[67]=F[67]+1:IF F[67]=10 THEN F[56]=1:Rstr="YOU SANK!"
670     IF R=56 AND F[35]=0 AND C[10]<>0 THEN Rstr=X1str+" GETS YOU!":F[56]=1
...
870     IF F[55]=1 THEN F[56]=1:Rstr="GRARGS HAVE GOT YOOU!":RETURN
880     IF R=29 AND F[48]=0 THEN Rstr="GRARGS WILL SEE YOU!":RETURN
890     IF R=73 OR R=42 OR R=9 OR R=10 THEN Rstr=X3str:F[55]=1:RETURN
900     IF C[8]=0 AND ((R=52 AND D=2) OR (R=31 AND D<>3))THEN Rstr="THE BOAT IS TOO HEAVY":RETURN
910     IF C[8]<>0 AND ((R=52 AND D=4) OR (R=31 AND D=3)) THEN Rstr="YOU CANNOT SWIM":RETURN
920     IF R=52 AND C[8] AND D=4 AND F[30]=0 THEN Rstr="NO POWER!":RETURN
930     IF R=41 AND D=3 AND F[31]=0 THEN Rstr="UIF CPBU JT TJOLJOH!":m4260:RETURN
940     IF R=33 AND D=1 AND F[32]=0 THEN Rstr="OGBAN'S BOAR BLOCK YOUR PATH":RETURN
950     IF ((R=3 AND D=2) OR (R=4 AND D=4)) AND F[45]=0 THEN Rstr=X5str:RETURN
960     IF R=35 AND C[13]<>R THEN Rstr="THE ICE IS BREAKING!":RETURN
970     IF R=5 AND (D=2 OR D=4) THEN m4310
980     IF R=4 AND D=4 THEN Rstr="PASSAGE IS TOO STEEP":RETURN
990     IF R=7 AND D=2 AND F[46]=0 THEN Rstr="A HUGE HOUND BARS YOUR WAY":RETURN
1000    IF (R=38 OR R=37) AND F[50]=0 THEN Rstr="JU JT UPP EBSL":m4260:RETURN
1010    IF R=49 AND D=2 AND F[54]=0 THEN ET Rstr="MYSTERIOUS FORCES HOLD YOU BACK":RETURN
1020    IF R=49 AND D=3 AND F[68]=0 THEN Rstr="YOU MEET OGBAN!!!":F[56]=1:RETURN
1030    IF R=38 ANF F[65]=0 THEN Rstr="RATS NIBBLE YOUR ANKLES":RETURN
1040    IF R=58 AND (D=1 OR D=4) AND F[66]=0 THEN Rstr="YOU GET CAUGHT IN THE WEBS!":RETURN
1050    IF R=48 AND D=4 AND F[70]=0 THEN Rstr="THE DOOR DOES NOT OPEN":RETURN
1060    IF R=40 AND F[47]=1 THEN F[68]=1
1070    IF R=37 AND D=4 AND Estr[37]="EW" THEN R=67:Rstr="THE PASSAGE WAS STEEP!":RETURN
...
1120    IF (Kstr="N" OR Kstr="U") AND D=1 THEN R=R-10
1130    IF Kstr="E" AND D=2 THEN R=R+1
1140    IF (Kstr="S" OR Kstr="D") AND D=3 THEN R=R+10
1150    IF Kstr="W" AND D=4 THEN R=R-1
1160    NEXT I:Rstr="OK"
1170    IF R=OM THEN Rstr="YOU CANNOT GO THAT WAY"
1180    IF ((OM=75 AND D=2) OR (OM=76 AND D=4)) THEN Rstr="OK. YOU CROSSED"
1190    IF F[29]=1 THEN F[39]=F[39]+1
1200    IF F[39]>5 AND F[29]=1 THEN Rstr="CPPUT IBWF XPSO PVU":m4260: F[29]=0:C[3]=81
1210    RETURN
1220    m3330:Rstr="OK":F[49]=0
1230    PRINT "YOU HAVE ";
1240    FOR I=1 TO G:READ Ostr:m3350:IF I=1 AND C[1]=0 AND F[44]=1 THEN Ostr="COIN"
1250    IF I=G AND C[5]=0 THEN GOTO 1270
1260    IF C[I]=0 THEN PRINT Ostr;",";:F[49]=1
1270    NEXT I:IF F[49]=0 THEN PRINT "NOTHING"
1280    PRINT:m3360:RETURN
end

def m1290
1290    IF H=6577 THEN Rstr="HOW?":RETURN
1300    IF H=4177 OR H=5177 THEN B=16:m2380:RETURN
1310    IF B=38 THEN Rstr="TOO HEAVY!":RETURN
1320    IF B=4 AND F[43]=0 THEN Rstr="IT IS FIRMLY NAILED ON!":RETURN
1330    CO=0:FOR I=1 TO G-1:IF C[I]=0 THEN CO=CO+1
1340    NEXT I:IF CO>13 THEN Rstr="YOU CANNOT CARRY ANYMORE":RETURN
1350    IF B>G THEN Rstr="YOU CANNOT GET THE "+Tstr:RETURN
1360    IF B=0 THEN RETURN
1370    IF C[B]<>R THEN Rstr="IT IS NOT HERE"
1380    IF F[B]=1 THEN Rstr="WHAT "+Tstr+"?"
1390    IF C[B]=0 THEN Rstr="YOU ALREADY HAVE IT"
1400    IF C[B]=R AND F[B]=0 THEN C[B]=0:Rstr="YOU HAVE THE "+Tstr
...
1480    IF B>0 THEN Rstr="NOTHING SPECIAL"
1490    IF B=46 OR B=88 THEN m2550
1500    IF H=8076 THEN Rstr="IT IS EMPTY"
1510    IF H=8080 THEN Rstr="AHA!":F[1]=0
1520    IF H=7029 THEN Rstr="OK":F[2]=0
1530    IF B=20 THEN Rstr="NBUDIFT JO QPDLFU":m4260:C[26]=0
1540    IF H=1648 THEN Rstr="THEREARE SOME LETTERS '"+Gstr[2]"'"
1550    IF H=7432 THEN Rstr="UIFZ BSF BQQMF USFFT":m4260:F[5]=0
1560    IF H=2134 OR H=2187 THEN Rstr="OK":F[16]=0
1570    IF B=35 THEN Rstr="IT IS FISHY!":F[17]=0
1580    IF H=3438 THEN Rstr="OK":F[22]=0
1590    IF H=242 THEN Rstr="A FADED INSCRIPTION"
1600    IF (H=1443 OR H=1485) AND F[33]=0 THEN Rstr="B HMJNNFSJOH GSPN UIF EFQUIT":m4260
1610    IF (H=1443 OR H=1485) AND F[33]=1 THEN Rstr="SOMETHING HERE...":F[12]=0
1620    IF H=2479 OR H=2444 THEN Rstr="THERE IS A HANDLE"
1630    IF B=9 THEN Rstr="UIF MBCFM SFBET 'QPJTPO'":m4260
1640    IF H=4055 THEN m3290
1650    IF H=2969 AND F[49]=1 THEN Rstr="VERY UGLY!"
1660    IF H=7158 OR H=7186 THEN Rstr="THERE ARE LOOSE BRICKS"
1670    IF R=49 THEN Rstr="VERY INTERESTING!"
1680    IF B=52 OR B=82 OR B=81 THEN Rstr="INTERESTING!"
1690    IF H=6978 THEN Rstr="THERE IS A WOODEN DOOR"
1700    IF H=6970 THEN Rstr="YOU FOUND SOMETHING":F[4]=0
1710    IF H=2066 THEN Rstr="A LARGE CUPBOARD IN THE CORNER"
1720    IF H=6865 OR H=6853 THEN Rstr="THERE ARE NINE STONES"
1730    IF H=248 THEN Rstr="A GBEFE XPSE - 'N S I T'":m4260
1740    RETURN
end

def m1750
1750    IF R=64 THEN Rstr="HE GIVES IT BACK!"
1760    IF H=6425 THEN m3210
1770    IF R=75 OR R=76 THEN Rstr="HE DOES NOT WANT IT"
1780    IF B=62 AND F[44]=0 THEN Rstr="YOU HAVE RUN OUT!"
1790    IF (H=7562 OR H=7662) AND F[44]>0 AND C[1]=0 THEN Rstr="HE TAKES IT":F[64]=1
1800    IF F[64]=1 THEN F[44]=F[44]-1
1810    IF B=1 THEN Rstr="HE TAKES THEM ALL!":C[1]=81:F[64]=1:F[44]=0
1820    IF H=2228 AND C[5]=81 THEN Rstr=XBstr+"NORTH":C[28]=81:R=12
1830    IF (H=2228 AND C[5]=81) OR H=225 THEN Rstr=XBstr+"NORTH":R=12
1840    IF (H=1228 AND C[5]=81) OR H=125 THEN Rstr=XBstr+"SOUTH":R=12
1850    IF R=7 OR R=33 THEN Rstr="HE EATS IT!":C[B]=81
1860    IF H=711 THEN F[46]=1:Rstr="HE IS DISTRACTED"
1870    IF H=385 OR H=3824 THEN Rstr="THEY SCURRY AWAY":C[B]=81:F[65]=1
1880    RETURN
1890    Rstr="YOU SAID IT"
1900    IF B=84 THEN Rstr="YOU MUST SAY THEM ONE BY ONE!":RETURN
1910    IF R<>47 OR B<71 OR B>75 OR C[27]<>0 THEN RETURN
1920    IF B=71 AND F[60]=0 THEN Rstr=X7str:F[60]=1:RETURN
1930    IF B=72 AND F[60]=1 AND F[61]=0 THEN Rstr=X8str:F[61]=1:RETURN
...
1970    RETURN
1980    IF B=3 THEN F[29]=1:Rstr="ZPV BSF JOWJTJCMF":F[55]=0:m4260
1990    IF B=20 THEN F[51]=1:Rstr="ZPV BSF EJTHVJTFE":F[55]=0:m4260
2000    RETURN
2010    IF B=2 OR B=14 THEN Rstr="NOTHING TO TIE IT TO!"
2020    IF H=7214 THEN Rstr="IT IS TIED":C[14]=72:F[53]=1
2030    IF H=722 THEN Rstr="OK":F[40]=1:C[2]=72
2040    RETURN
2050    IF H=1547 AND F[38]=1 THEN Rstr="ALL RIGHT":R=16
2060    IF B=14 OR B=2 THEN Rstr="NOT ATTACHED TO ANYTHING!"
2070    IF H=5414 AND C[14]=54 THEN Rstr="YOU ARE AT THE TOP"
2080    IF H=7214 AND F[53]=1 THEN Rstr="GOING DOWN":R=71
2090    IF H=722 AND F[40]=1 THEN R=71:Rstr="IT IS TORN":C[2]=81:F[40]=0
2100    IF H=7114 AND F[53]=1 THEN C[14]=71:F[53]=0:Rstr="IT FALLS DOWN-BUMP!"
2110    RETURN
2120    IF H=522 THEN Rstr="OK":F[30]=1
2130    IF B=1 OR B=62 OR B=5 OR B=28 OR B=11 OR B=24 THEN m1750
2140    IF H=416 THEN Rstr="ZPV IBWF LFQU BGMPBU":F[31]=1:m4260:RETURN
2150    IF H=4116 THEN Rstr="IT IS NOT BIG ENOUGH!":RETURN
...
2230    IF H=2030 THEN F[9]=0:Rstr="OK"
2240    IF H=6030 THEN Rstr="OK":F[3]=0
2250    IF H=2444 OR H=1870 THEN Rstr="YOU ARE NOT STRONG ENOUGH"
2260    IF H=3756 THEN Rstr="A PASSAGE!":Estr[37]="EW"
2270    IF H=5960 THEN m3260
2280    IF H=6970 THEN Rstr="IT FALLS OFF ITS HINGES"
2290    IF H=4870 THEN Rstr="IT IS LOCKED"
2300    RETURN
2310    IF B>G THEN Rstr="IT DOES NOT BURN"
2320    IF B=26 THEN Rstr="YOU LIT THEM"
2330    IF H=3826 THEN Rstr="NOT BRIGHT ENOUGH"
2340    IF (B=23 OR H=6970) AND C[26]<>0 THEN Rstr="OP NBUDIFT":m4260
2350    IF B=23 AND C[26]=0 THEN Rstr="A BRIGHT "+Vstr:F[50]=1
2360    IF H=6970 AND C[26]=0 THEN F[43]=1:Rstr="IT HAS TURNED TO ASHES"
2370    RETURN
end

def m2380
2380    IF (B=16 OR B=6) AND (R=41 OR R=51) THEN Rstr="YOU CAPSIZED!":F[56]=1
2390    IF H=6516 AND C[16]=0 THEN Rstr="IT IS NOW FULL":F[34]=1
2400    IF H=656 THEN Rstr="IT LEAKS OUT!"
2410    RETURN
2420    IF B<>22 OR R<>15 THEN Rstr="DOES NOT GROW!":RETURN
2430    Rstr="OK":F[34]=1
2440    RETURN
end

def m2450
2450    IF B=22 AND F[37]=1 AND F[34]=1 THEN Rstr=X2str:F[38]=1:m4260
2460    RETURN
end

def m2470
2470    IF B=7 OR B=18 THEN Rstr="THWACK!"
2480    IF H=5818 THEN Rstr="YOU CLEARED THE WEBS":F[66]=1
2490    IF H=187 THEN Rstr="THE DOOR BROKE!":Estr[18]="NS":Estr[28]="NS"
2500    IF H=717 THEN Rstr="YOU BROKE THROUGH":Estr[71]="N"
2510    RETURN
2520    IF B=16 THEN B=22:m2450
2530    IF H=499 THEN Rstr="WHERE?"
2540    RETURN
end

def m2550
2550    IF H=4337 THEN VB=2:m800:RETURN
2560    IF R=36 THEN Rstr="YOU FOUND SOMETHING":F[13]=0
2570    RETURN
2580    IF R=76 THEN VB=4:m800:RETURN
2590    IF R=75 THEN VB=2:m800
2600    RETURN
2610    IF (B=3 AND F[29]=1) THEN Rstr="TAKEN OFF":F[29]=0
2620    IF (B=20 AND F[51]=1) THEN Rstr="OK":F[51]=0
2630    IF B=36 OR B=50 THEN m2950
2640    RETURN
2650    IF H=3859 OR H=3339 OR H=1241 OR H=2241 OR H=751 THEN Rstr="WITH WHAT?"
2660    RETURN
2670    IF H=2340 THEN Rstr="IT GOES ROUND"
2680    IF H=2445 THEN Rstr="UIF HBUFT PQFQ, UIF QPPM FNQUJFT":F[33]=1:m4260
2690    RETURN
2700    IF R=14 OR R=51 THEN Rstr="YOU HAVE DROWNED":F[56]=1
2710    RETURN
...
2750    IF H=418 OR H=518 THEN Rstr="YOU DROWNED!":F[56]=1
2760    IF B=8 AND F[30]=1 THEN C[2]=R
2770    IF B=16 AND F[34]=1 THEN Rstr="YOU LOST THE WATER!":F[34]=0
2780    IF B=2 AND F[30]=1 THEN F[30]=0
2790    RETURN
2800    IF B=62 AND F[44]=0 THEN Rstr="YOU DO NOT HAVE ANY"
2810    IF H=5762 AND C[1]=0 AND F[44]>0 THEN m3230
2820    RETURN
2830    IF B=0 OR B>G THEN RETURN
2840    Rstr="DID NOT GO FAR!":C[B]=R
2850    IF H=3317 THEN Rstr="ZPV DBVHIU UIF CPBS":F[32]=1:m4260
2860    RETURN
end

def m2870
2870    IF B=10 THEN Rstr+"B OJDF UVOF":m4260
2880    IF H=5233 THEN Rstr="WHAT WITH?"
2890    IF B=83 THEN Rstr="HOW, O MUSICAL ONE?"
2900    IF H=5610 THEN F[35]=1:Rstr=X1str+" IS FREE!":Estr[56]="NS"
2910    RETURN
2920    IF B=0 OR B>G THEN RETURN
2930    IF B=5 OR B=24 THEN Rstr="YUM YUM!":C[B]=81
2940    RETURN
end

def m2950
2950    IF R=4 AND B=50 THEN F[45]=1:Rstr="YOU REVEALED A STEEP PASSAGE"
2960    IF R=3 AND B=50 THEN Rstr="YOU CANNOT MOVE RUBBLE FROM HERE"
2970    IF H=7136 THEN Rstr="THEY ARE WEDGED IN!"
2980    RETURN
2990    IF (B=67 OR B=68) AND C[9]=0 AND R=49 THEN Rstr="OK":F[47]=1
3000    RETURN
3010    IF R<>27 OR B<>63 THEN RETURN
3020    PRINT:PRINT "HOW MANY TIMES?":INPUT MR:IF MR=0 THEN PRINT "A NUMBER":GOTO 3020
3030    IF MR=F[42] THEN Rstr="A ROCK DOOR OPENS":Estr[27]="EW":RETURN
3040    Rstr="ZPV IBWF NJTUSFBUFE UIF CFMM!":F[56]=1:m4260:RETURN
3050    IF H=5861 THEN H=5818:m2470
3060    RETURN
end

def m3070
3070    IF (H=4864 OR H=4819) AND C[19]=0 THEN Rstr=X6str:F[63]=1:m4260
3080    IF B=27 THEN m1290
3090    RETURN
3100    IF H=7549 OR H=7649 THEN Rstr="WHAT WITH?"
3110    IF B=1 OR B=62 THEN m1750
3120    RETURN
3130    IF H=4870 AND C[21]=0 THEN Rstr="THE KEY TURNS!":F[70]=1
3140    RETURN
3150    IF H=1870 THEN Rstr="HOW?"
3160    RETURN
3170    IF R=48 THEN Rstr="HOW?"
3180    RETURN
...
3270    Rstr="WRONG!":IF CN=F[41] THEN Rstr="IT OPENS":F[21]=0
3280    RETURN
end
...
4270    Cstr=MIDstr(Rstr,I,1):IF Cstr<"A" THEN Zstr=Zstr+Cstr:GOTO 4300
4280    C=ASC[Cstr]-1:IF C=64 THEN C=90
...
4370    IF Wstr="G" THEN F[56]=1:RETURN
4380    IF Jstr<>Gstr[MP] THEN NG=NG+1:GOTO 4320
4390    RETURN
end
...
4540    IF Fstr="N" THEN Lstr="S"
4550    IF Fstr="S" THEN Lstr="N"
4560    IF Fstr="E" THEN Lstr="W"
4570    IF Fstr="W" THEN Lstr="E"
...
4850    IF MIDstr(Jstr,I,1)=" " AND LL>EL THEN PRINT MIDstr(Jstr,LP,LS-LP):LL=I-LS:LP=LS+1
4860    IF MIDstr(Jstr,I,1)=" " THEN LS=I
4870    LL=LL+1:NEXT I
